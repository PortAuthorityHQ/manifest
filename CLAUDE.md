...
Pure Python re-implementation of manifest-core in `sdk/python/`. No FFI or Rust dependency.

```
sdk/python/
├── pyproject.toml              — Package config (name: manifest-sdk)
├── manifest_sdk/
│   ├── __init__.py             — Public exports + high-level Manifest class
│   ├── hashing.py              — SHA-256 helpers (sha256, sha256_hex)
│   ├── signer.py               — Ed25519 signing (32-byte seed files, ed25519:<base64> format)
│   ├── merkle.py               — Append-only Merkle tree (matches Rust impl exactly)
...
Key: receipts generated by the Python SDK must be verifiable by the Rust CLI (`manifest verify`). Same signing order, same canonical JSON, same Merkle tree rules.
