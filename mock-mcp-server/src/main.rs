...
//!
//! Supports two modes:
//!   - **stdio** (default): Reads newline-delimited JSON-RPC from stdin, responds on stdout
//!   - **HTTP**: `--http <port>` starts an Axum server on the given port
//!
//! Supports:
...
async fn run_http(port: u16) {
    let app = axum::Router::new().route(
        "/mcp",
        axum::routing::post(http_handler),
    );

    let addr = std::net::SocketAddr::from(([127, 0, 0, 1], port));
    eprintln!("Mock MCP server (HTTP) listening on http://{addr}/mcp");

    let listener = tokio::net::TcpListener::bind(addr).await.unwrap();
    axum::serve(listener, app).await.unwrap();
}

async fn http_handler(body: String) -> axum::response::Response {
